...
const PostAPI = () => {
  ...
  const checkEmptyFields = (apiDetails) => {
    ...
    if (apiDetails.codeProvided.trim() === "")
      emptyFields.push("예시코드 제공 여부");
    if (
      apiDetails.codeProvided === "제공" &&
      apiDetails.examplecode.trim() === ""
    )
      emptyFields.push("예시코드");
    ...
    reader.onload = (event) => {
      try {
        ...
        setApiDetails({
          ...
          requests: json.apis[0].request
            ? json.apis[0].request.map((req) => ({
                name: req.parameter,
                type: req.type,
            ...
            : [],
          responses: json.apis[0].response
            ? json.apis[0].response.map((res) => ({
                name: res.field,
                type: res.type,
                description: res.description,
              }))
  ...
  // API 호출 함수 추가
  const getFisheryValueFutureOceanicData = async (
    serviceKey,
    longitude,
    latitude,
    date,
          ...
          <S.FormContainer>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <S.RadioGroup>
                  <label>
                    <input
                      ...
                      name="codeProvided"
                      value="제공"
                      checked={apiDetails.codeProvided === "제공"}
                      onChange={handleInputChange}
                    />{" "}
                    제공
                  </label>
                  <label>
                    <input
                      ...
                      name="codeProvided"
                      value="미제공"
                      checked={apiDetails.codeProvided === "미제공"}
                      onChange={handleInputChange}
                    />{" "}
                    미제공
                  </label>
                ...
                <S.RadioGroup>
                  ...
                  <label>
                    <input
                      ...
                      name="method"
                      value="POST"
                      checked={apiDetails.method === "POST"}
                      onChange={handleInputChange}
                    />{" "}
                    POST
                  </label>
                  ...
                  <label>
                    <input
                      ...
                      name="method"
                      value="PATCH"
                      checked={apiDetails.method === "PATCH"}
                      onChange={handleInputChange}
                    />{" "}
                    PATCH
                  </label>
                  <label>
                    <input
                      ...
                      name="method"
                      value="DELETE"
                      checked={apiDetails.method === "DELETE"}
                      onChange={handleInputChange}
                    />{" "}
                    DELETE
                  </label>
              ...
              <button type="button" onClick={addRequest}>
                {editType === "request" && editIndex !== null ? "수정" : "추가"}
              </button>
              <div className="request-list">
                ...
                {apiDetails.requests.map((ep, index) => (
                  <S.Item key={index}>
                    ...
                    <button
                      type="button"
                      onClick={() => handleDelete(index, "request")}
                    >
                      삭제
                    </button>
                  </S.Item>
              ...
              <button type="button" onClick={addResponse}>
                {editType === "response" && editIndex !== null
                  ? "수정"
                  : "추가"}
              </button>
              <div className="response-list">
                ...
                {apiDetails.responses.map((ep, index) => (
                  <S.Item key={index}>
                    ...
                    <button
                      type="button"
                      onClick={() => handleEdit(index, "response")}
                    >
                      수정
                    </button>
                    <button
                      type="button"
                      onClick={() => handleDelete(index, "response")}
                    >
                      삭제
                    </button>
                  </S.Item>
